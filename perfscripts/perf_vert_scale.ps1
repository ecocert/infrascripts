...
function Show-Menu
{
     ...
     Write-Host "10: Enter '10' To Remove interfaces From Linux VM"
     Write-Host "11: Enter '11' To Remove Linux VMS from the VC inventory"
     Write-Host "12: Enter '12' To Clear Infrastructure Setup"
    ...
    do
    {
         ...
         switch ($input)
         {
               ...
               '10' {
                  [int]$start = Read-Host -Prompt "Enter Starting Linx VM"
                  [int]$end  = Read-Host -Prompt "Enter Ending Linux VM"
                  [int]$numOfInf = Read-Host -Prompt "Enter number of interfaces to be removed"
                  removeInterfacesFromLinuxVM $start $end $numOfInf 'VDS_vertical_scale_pg'
               }
...
Function removeInterfacesFromLinuxVM([int]$start, [int]$end, [int] $numOfInf, [string] $networkName) {
