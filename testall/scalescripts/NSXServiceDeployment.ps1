param($index,$servicename,$method)
...
Function deployService([string] $method, [string] $clusterID, [string] $datastoreID, [string] $serviceID, [string] $dvportgroupID, [string] $ippoolID)
{
...
try
{
    ...
    $return = deployService $method $cluster_moref $datastore_moref $serviceID $portgroup_moref $ippoolid
    if( $return -eq $false)
    {
